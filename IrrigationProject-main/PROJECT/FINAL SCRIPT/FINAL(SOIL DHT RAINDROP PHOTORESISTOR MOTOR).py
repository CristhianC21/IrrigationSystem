...
def photoresistorLCD(LcdPhotoresistor):
...
def lcdscreen():
    ...
    while(True):
      ...
      # lcd.clear()
        ...
        lcd.message( str(moistureLCD(soilMoisture())) + str(soilMoisture())+'%' )# display CPU temperature
        lcd.setCursor(1,1)  # set cursor position
        lcd.message( "T:"+ str(DHT11sensor()) +'C' )# display CPU temperature
        print("2. ROOM TEMPERATURE : %.1f "%(DHT11sensor()))
        lcd.setCursor(10,0)
        lcd.message(str(photoresistorLCD(photoresistorLed())) )
        printRaindrop(raindropSensor())
        sleep(1)
        
        motor(soilMoisture(),DHT11sensor(),photoresistorLed(),raindropSensor())
